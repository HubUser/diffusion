      subroutine funNTet2(s, numT, arrayT, arrayG, valueKin, valueWin, v
     *alueN, Nmax, coefTan)
      real s, valueKin, valueWin
      integer numT
      real arrayT(numT), arrayG(numT)
      real valueN, Nmax, coefTan
      real zn, c, co2, si2, n2, F, dZ, Z, X, Y2, Sq
      real cosTetR, Tetr, fun(numT)
C
      valueN = 0
      X=s*s/valueWin**2
...
C      c = valueKin**2*valueKin/valueWin*(0.5*F+n2)/n2*0.5/(3.14159)**2
     ...
     *   .and. tan(arrayT(i))**2<coefTan*(1.0/cosTetR**2-1)
...
C
      do i = 1, numT-1
       valueN = valueN + 0.5*(arrayT(i+1)-arrayT(i))*(fun(i)+fun(i+1))
      end do
